...
generate_random_cluster <- function(){
  repeat{
    ...
    for(i in 1:numsegments){
      clusters[i] = sample(1:numclusters, 1, replace=F)
    }
...
#-----------Validate the minimal numbers observations on each cluster-----------
...
remove_high_vif <- function(fit,newnames){
  if(length(grep("NA",newnames))<(length(newnames)-2)){
    vif_fit = vif(fit)
    if(!is.nan(max(vif_fit))){
      if(max(vif_fit)>max_vif){
...
for (numclusters in min_clusters:max_clusters){
  ...
  print_optimization(0, best_eval[1], best_eval[2], best_eval[3])
  j = 1
  while (temp > temp_min){
    i = 1
